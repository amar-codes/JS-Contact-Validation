...
function checkForm() {
    ...
    if (phoneNumber) {
        if (phoneNumber.isValid()) {
            document.getElementById("form-number").style.borderBottom = "3px solid green";
            console.log(phoneNumber.isValid());
            document.getElementById("error-number").style.display = "none";
        }
    } else {
        document.getElementById("form-number").style.borderBottom = "3px solid red";
        document.getElementById("error-number").style.display = "block";
        document.getElementById("error-number").style.color = "red"
    }
    ...
    let imev = document.getElementById("form-name").value;
    let ime = document.getElementById("form-name");
    ...
    if (!filter.test(email)) {
        document.getElementById("form-email").style.borderBottom = "3px solid red";
        document.getElementById("error-email").style.display = "block";
        document.getElementById("error-email").style.color = "red"
    } else {
        document.getElementById("form-email").style.borderBottom = "3px solid green";
        document.getElementById("error-email").style.display = "none";
    }

    if (phoneNumber) {
        if (phoneNumber.isValid() && regName.test(imev) && filter.test(email)) {
            console.log("Uspjesno popunjena forma");
            document.getElementById("error-name").remove();
            document.getElementById("error-number").remove();
            document.getElementById("info-form").innerHTML = "Uspješno popunjena forma";
